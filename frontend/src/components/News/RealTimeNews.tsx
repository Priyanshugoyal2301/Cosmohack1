...
const RealTimeNews: React.FC = () => {
  ...
  const fetchNews = useCallback(async (category: string = 'all', search: string = '', forceRefresh: boolean = false) => {
    try {
      setLoading(true);
      ...
      if (search) {
        ...
        articles = await searchNews(search, forceRefresh);
      } else {
        // Fetch all latest news
        articles = await fetchAllNews(forceRefresh);
      }
