...
export async function fetchAllNews(forceRefresh: boolean = false): Promise<NewsArticle[]> {
  // Return cached articles if available and not forcing refresh
  if (cachedArticles && !forceRefresh) {
    console.log(`📦 Using cached news (cached articles: ${cachedArticles.length})`);
...
export async function searchNews(query: string, forceRefresh: boolean = false): Promise<NewsArticle[]> {
  console.log(`🔍 Searching for: "${query}"`);
  
  const allArticles = await fetchAllNews(forceRefresh);
